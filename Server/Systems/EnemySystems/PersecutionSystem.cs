...
{
    private readonly QueryMeta _queryMeta = new QueryMeta().WithAll<Speed, Target, NetworkTransform>();
    ...
    public override void GameUpdate(long tick, long predictTick)
    {
        ...
        {
            if (!target.TargetEntity.HasValue || !HasComponent<NetworkTransform>(target.TargetEntity.Value))
                return;
